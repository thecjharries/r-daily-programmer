...
fn mccarthy_91(start: i32, output: &mut impl Write) -> i32 {
    if 100 < start {
        if 101 == start {
    ...
    .unwrap();
    return mccarthy_91(mccarthy_91(start + 11, output), output);
}
...
mod tests {
    ...
    fn test_mccarthy_91() {
        ...
        assert_eq!(mccarthy_91(99, &mut output), 91);
        ...
        output.clear();
        assert_eq!(mccarthy_91(101, &mut output), 91);
        assert_eq!(&output, b"91 since 101 is greater than 100\n");
        output.clear();
        assert_eq!(mccarthy_91(91, &mut output), 91);
        ...
        output.clear();
        assert_eq!(mccarthy_91(102, &mut output), 92);
